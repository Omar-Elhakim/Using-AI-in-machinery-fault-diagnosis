...
dataset_categories = {
    ...
    4: "lxc",  # Classifying LXC faults                            (binary)
...
dataset_descriptions = {
    1: {
        "Faults classified": "AirGap, Electrical and SoftFoot problems",
        "Channels taken": "Motor channels only",
        "Labels array": "Binary",
    },
    2: {
        "Faults classified": "BearingFault, BearingWear, BearingLubrication",
        "Channels taken": "According to label",
        "Labels array": "Binary",
    },
    3: {
        "Faults classified": "Flow problem",
        "Channels taken": "Machine channels only",
        "Labels array": "Binary",
    },
    4: {
        "Faults classified": "LXC, XC, Losseness",
        "Channels taken": "All",
        "Labels array": "Binary",
    },
    5: {
        "Faults classified": "LXC, Unbalance, BaseProblems",
        "Channels taken": "All",
        "Labels array": "multiclass multilabel",
    },
    6: {
        "Faults classified": "Coupling problems",
        "Channels taken": "Drive end channels only",
        "Labels array": "Binary",
    },
    7: {
        "Faults classified": "All",
        "Channels taken": "All",
        "Labels array": "multiclass multilabel",
    },
    8: {
        "Faults classified": "Status",
        "Channels taken": "All",
        "Labels array": "multiclass",
    },
}
...
def separate_data(ts, labels, split_method):
    ...
    if split_method == "channel":
        ...
        raise Exception(
            "Invalid split type: choose from [channel, bearing, machine, None]"
        )
    ...
    if dataset_category == 1:
        ...
        new_labels = [
            (
                1
                if any(
                    motor_fault in fault
                    for fault in faults_list
                    for motor_fault in faults
                )
                else 0
            )
            for faults_list in labels
        ]
        new_ts = ts[:, :6, :]
        ...
        new_labels = [
            (
                1
                if any(
                    lxc_fault in fault for fault in faults_list for lxc_fault in faults
                )
                else 0
            )
            for faults_list in labels
        ...
        new_labels = [
            (
                1
                if any(
                    coupling_fault in fault
                    for fault in faults_list
                    for coupling_fault in faults
                )
                else 0
            )
            for faults_list in labels
...
def apply_sliding_window(
    ts_3d, labels, window_size=1024, overlap_pct=0.1, with_fft=False
):
    ...
    for ts, label in zip(ts_3d, labels):
        ...
        for i in range(num_windows):
            ...
            if with_fft == True:
                x_temp[i] = np.apply_along_axis(
                    calculate_fft, arr=ts[:, start:end], axis=1
                )
            else:
